    ...
    public partial class AssemblyPage : ContentPage
    {
        ...
        protected override void OnAppearing()
        {
            ...
            if (v.Id != 0)
            {
                ...
                // }
                ListPartsInAssembly.ItemsSource = GetAssemblyFromId(v.Id).Parts;
            }
        ...
        private void SaveAssembly(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(assembly.Name))
            {
                ...
                {
                    var assemblyUpd = GetAssemblyFromId(assembly.Id);
                    assemblyUpd.Name = assembly.Name;
                    ChangeAssembly(assemblyUpd);
                }
            }
        ...
        {
            var assembly = (Assembly)BindingContext;
            HttpRequestData.DeleteAssembly(assembly);
            this.Navigation.PopAsync();
        }
        ...
        private async void AddPart(object sender, EventArgs e)
        {
            ...
            {
                AddAssembly(assembly);
                assembly = GetAllAssemblies().Find(x => x.Name == assembly.Name);
                Console.WriteLine(1241245123512);
            }
            ...
            if (assembly.Id != 0)
            {
                var tempAssembly = GetAssemblyFromId(assembly.Id);
                var partL = tempAssembly.Parts.Find(x => x.Detail.Name == detailName);
                if (partL != null)
                {
            ...
            var allDetails = GetAllDetails();
            var detail = allDetails.Find(x => x.Name == detailName);
            if (detail == null)
            {
        ...
        private async void EditPart(object sender, EventArgs e)
        {
            ...
            if (quantityNew == "0" || quantityNew == "" || !int.TryParse(quantityNew, out var numericValue))
                return;
            var assemblyUpd = GetAssemblyFromId(assembly.Id);
            assemblyUpd.Parts.Find(x => x.Detail.Name == detailName).Quantity = int.Parse(quantityNew);
            ChangeAssembly(assemblyUpd);
            OnAppearing();
        }
        ...
        private void DeletePart(object sender, EventArgs e)
        {
            ...
            var assemblyUpd = GetAssemblyFromId(assembly.Id);
            var detailName = ((MenuItem)sender).CommandParameter.ToString();
            assemblyUpd.Parts.RemoveAll(x => x.Detail.Name == detailName);
            ChangeAssembly(assemblyUpd);
            OnAppearing();
        }
    }
